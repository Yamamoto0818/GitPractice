...
public class kadai17LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 入力されたIDをもとにソルトを取得する。
		String salt = kadai16DAO.getSalt(mail);
		
		// 取得したソルトがnullの場合は対象のユーザがいないので、Errorでログイン画面に戻す
		if(salt == null) {
			String view = "WEB-INF/view/kadai17login.jsp?error=1";
		...
		kadai16 account = (kadai16) kadai16DAO.login(mail, hashedPw);
		...
		if(account == null) {
			...
			String view = "WEB-INF/view/kadai17menu.jsp";
			RequestDispatcher dispatcher = request.getRequestDispatcher(view);
			dispatcher.forward(request, response);
		}
	}
